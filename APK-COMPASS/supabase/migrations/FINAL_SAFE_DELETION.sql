-- FINAL SAFE DELETION SCRIPT v3
-- Checks if table exists before delete to avoid errors.
DO $$
DECLARE tgt_email TEXT := 'zbigniew.twardowski@b2bnetwork.pl';
tgt_id UUID;
prof_id UUID;
BEGIN
SELECT id INTO tgt_id
FROM auth.users
WHERE email = tgt_email;
SELECT id INTO prof_id
FROM profiles
WHERE id = tgt_id;
IF tgt_id IS NOT NULL THEN RAISE NOTICE 'Attempting to delete User ID: %',
tgt_id;
-- 1. DELETE CANDIDATE ROW (Primary Blocker)
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'candidates'
) THEN
DELETE FROM candidates
WHERE user_id = tgt_id;
DELETE FROM candidates
WHERE email = tgt_email;
END IF;
-- 2. Clean up dependencies SAFELY
-- Notifications
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'notifications'
) THEN
DELETE FROM notifications
WHERE user_id = tgt_id;
END IF;
-- Contracts
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'contracts'
) THEN
DELETE FROM contracts
WHERE consultant_id = tgt_id
    OR created_by = tgt_id;
END IF;
-- Documents (The one causing errors)
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'app_documents'
) THEN
DELETE FROM app_documents
WHERE owner_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'document_versions'
) THEN
DELETE FROM document_versions
WHERE uploaded_by = tgt_id;
END IF;
-- Loyalty
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'loyalty_transactions'
) THEN
DELETE FROM loyalty_transactions
WHERE user_id = tgt_id;
END IF;
-- Centrala & Referrals
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'centrala_benefit_declarations'
) THEN
DELETE FROM centrala_benefit_declarations
WHERE profile_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'centrala_referrals'
) THEN
DELETE FROM centrala_referrals
WHERE referrer_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'centrala_equipment_requests'
) THEN
DELETE FROM centrala_equipment_requests
WHERE profile_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'centrala_invoices'
) THEN
DELETE FROM centrala_invoices
WHERE profile_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'project_referrals'
) THEN
DELETE FROM project_referrals
WHERE referrer_user_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'favorite_projects'
) THEN
DELETE FROM favorite_projects
WHERE user_id = tgt_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'audit_logs'
) THEN
DELETE FROM audit_logs
WHERE user_id = tgt_id;
END IF;
-- Communicator
IF prof_id IS NOT NULL THEN IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'messages'
) THEN
DELETE FROM messages
WHERE sender_id = prof_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'conversation_participants'
) THEN
DELETE FROM conversation_participants
WHERE user_id = prof_id;
END IF;
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'conversations'
) THEN
DELETE FROM conversations
WHERE owner_id = prof_id;
END IF;
END IF;
-- 3. Delete Profile & User
IF EXISTS (
    SELECT
    FROM pg_tables
    WHERE schemaname = 'public'
        AND tablename = 'profiles'
) THEN
DELETE FROM profiles
WHERE id = tgt_id;
END IF;
DELETE FROM auth.users
WHERE id = tgt_id;
RAISE NOTICE 'SUCCESS: User deleted.';
ELSE RAISE NOTICE 'User not found.';
END IF;
END $$;